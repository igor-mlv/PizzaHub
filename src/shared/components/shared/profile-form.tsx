...
export const ProfileForm: React.FC<Props> = ({ data }) => {
  ...
  const onSubmit = async (data: TFormRegisterValues) => {
    try {
      ...
      toast.error('Information updated 📝', {
        icon: '✅',
      });
    } catch (error) {
      return toast.error('Error updating information', {
        icon: '❌',
      });
    }
  };
    ...
    <Container className="my-10 flex flex-col items-center">
      ...
      <FormProvider {...form}>
        <form className="flex flex-col gap-5 w-96 mt-10" onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <FormInput type="password" name="confirmPassword" label="Confirm new password" required />
